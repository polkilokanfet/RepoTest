...
namespace HVTApp.UI.ViewModels
{
    ...
    public class OfferViewModel : UnitedViewModel<OfferWrapper, Offer, OfferUnit, OfferUnitWrapper, AfterSaveOfferEvent>
    {
    ...
    public class ProjectViewModel : UnitedViewModel<ProjectWrapper, Project, SalesUnit, SalesUnitWrapper, AfterSaveProjectEvent> {
        public ProjectViewModel(IUnityContainer container) : base(container)
        {
        }
    }

    public abstract class UnitedViewModel<TWrapper, TEntity, TUnit, TUnitWrapper, TAfterSaveEntityEvent> :
        BaseDetailsViewModel<TWrapper, TEntity, TAfterSaveEntityEvent>
        where TEntity : class, IBaseEntity
        where TUnit : class, IBaseEntity
        ...
        where TAfterSaveEntityEvent : PubSubEvent<TEntity>, new()
    {
        ...
        protected UnitedViewModel(IUnityContainer container) : base(container)
        {
        }
        ...
        private async void ChangeProductCommand_Execute(IProductUnitsGroup group)
        {
            ...
            if (product == null || product.Id == group.Product.Id) return;
            group.Product =
                await WrapperDataService.GetWrapperRepository<Product, ProductWrapper>().GetByIdAsync(product.Id);
            RefreshPrices();
        }
        ...
        private async void ChangeFacilityCommand_Execute(IProductUnitsGroup group)
        {
            ...
            if (facility == null) return;
            group.Facility =
                await WrapperDataService.GetWrapperRepository<Facility, FacilityWrapper>().GetByIdAsync(facility.Id);
        }
